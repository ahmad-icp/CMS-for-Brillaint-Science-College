...
from app.modules.examination.schemas import (
    AssessmentComponentCreate,
    ExamCreate,
    ExamHallCreate,
    ExamScheduleCreate,
    ExamTypeCreate,
    InvigilatorAssignmentCreate,
)
...
def test_schedule_publish_and_lock_exam(db_session):
    ...
    schedule = service.schedule_exam(
        ExamScheduleCreate(
            exam_id=exam.id,
            subject_id=subject.id,
            hall_id=hall.id,
    ...
    assignment = service.assign_invigilator(
        InvigilatorAssignmentCreate(schedule_id=schedule.id, teacher_id="t-1", teacher_name="Ada Teacher"),
        "principal-1",
    )
