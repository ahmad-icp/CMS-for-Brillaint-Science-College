...
def test_manual_challan_and_partial_payment(db_session):
    svc=FeeService(db_session,'college-a')
    head=svc.create_head(FeeHeadCreate(code='TUI', name='Tuition', head_type=FeeHeadType.TUITION))
    ch=svc.create_manual_challan(ManualChallanCreate(student_id='stu-1', billing_period='2026-07', issue_date=date(2026,7,1), due_date=date(2026,7,10), lines=[ChallanLineCreate(fee_head_id=head.id, amount=Decimal('1000'))]))
...
def test_duplicate_payment_is_flagged_without_double_counting(db_session):
    ...
    ch=svc.create_manual_challan(ManualChallanCreate(student_id='stu-1', billing_period='2026-FALL', issue_date=date(2026,8,1), due_date=date(2026,8,15), lines=[ChallanLineCreate(fee_head_id=head.id, amount=Decimal('500'))]))
