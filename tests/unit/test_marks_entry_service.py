...
from app.modules.marks_entry.schemas import (
    BulkMarksUpsert,
    MarksBatchCreate,
    MarksEntryUpsert,
    MarksImportRequest,
...
def test_bulk_submit_lock_and_unlock_marks(db_session):
    ...
    entries = service.bulk_upsert(
        batch.id,
        BulkMarksUpsert(entries=[MarksEntryUpsert(student_id=student.id, marks_obtained=88)]),
        "teacher-1",
    )
    ...
    unlocked = service.unlock_batch(
        batch.id,
        UnlockRequest(reason="Approved correction request"),
        "principal-1",
    )
...
def test_marks_validation_rejects_above_maximum(db_session):
    ...
    with pytest.raises(HTTPException) as exc:
        service.upsert_entry(
            batch.id,
            MarksEntryUpsert(student_id=student.id, marks_obtained=101),
            "teacher-1",
        )
...
def test_csv_import_and_audit(db_session):
    service, batch, student = prepared_batch(db_session)
    imported = service.import_marks(
        batch.id,
        MarksImportRequest(
            format="csv",
            content=f"student_id,marks_obtained,remarks\n{student.id},77,Good",
        ),
        "teacher-1",
    )
