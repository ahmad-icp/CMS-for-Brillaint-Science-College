...
def test_certificate_approval_issue_and_verification(db_session):
    ...
    request = service.request_certificate(CertificateRequestCreate(template_id=template.id, certificate_type=CertificateType.BONAFIDE, student_id="student-1", purpose="Scholarship", context={"name": "Ayesha"}))
...
def test_document_approval_workflow(db_session):
    service = CertificateService(db_session, "college-a")
    document = service.upload_document(DocumentCreate(owner_type="student", owner_id="student-1", document_type=RepositoryDocumentType.ACADEMIC, title="Matric Result", file_path="storage/documents/matric.pdf"))
