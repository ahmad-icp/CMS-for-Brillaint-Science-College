...
def create_student(db_session, section_name="A"):
    return StudentService(db_session, "college-a").create_student(
        StudentCreate(
...
def test_mark_and_finalize_attendance(db_session):
    ...
    records = service.mark_records(
        session.id,
        BulkAttendanceMark(records=[AttendanceRecordMark(student_id=student.id, status="present")]),
        "teacher-1",
    )
    ...
    with pytest.raises(HTTPException) as exc:
        service.mark_records(
            session.id,
            BulkAttendanceMark(records=[AttendanceRecordMark(student_id=student.id, status="absent")]),
            "teacher-1",
        )
...
def test_attendance_summary_counts_late_as_present(db_session):
    ...
    service.mark_records(
        session.id,
        BulkAttendanceMark(
            records=[AttendanceRecordMark(student_id=student.id, status="late", late_minutes=5)]
        ),
        "teacher-1",
    )
