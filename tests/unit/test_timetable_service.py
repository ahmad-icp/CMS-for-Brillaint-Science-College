...
def seed(db_session):
    ...
    department = academic.create_department(
        DepartmentCreate(institution_id=institution.id, name="Science", code="SCI")
    )
    session = academic.create_session(
        AcademicSessionCreate(
            name="2026-2027",
    ...
    program = academic.create_program(
        ProgramCreate(department_id=department.id, name="Middle Science", code="MS", duration_years=3)
    )
    klass = academic.create_class(
        AcademicClassCreate(program_id=program.id, session_id=session.id, name="Class 6")
    )
    section = academic.create_section(
        SectionCreate(class_id=klass.id, name="A", capacity=40, enrolled_count=30)
    )
    subject = academic.create_subject(
        SubjectCreate(department_id=department.id, code="MATH", name="Mathematics", weekly_periods=5)
    )
    academic.create_subject_allocation(
        SubjectAllocationCreate(class_id=klass.id, subject_id=subject.id, weekly_periods=5)
    )
    academic.create_teacher_assignment(
        TeacherAssignmentCreate(
            teacher_id="t-1",
    ...
    slot = timetable.create_time_slot(
        TimeSlotCreate(name="P1", start_time=time(8, 0), end_time=time(8, 45), sort_order=1)
    )
    timetable.set_working_day(WorkingDayCreate(session_id=session.id, weekday=1, is_working=True))
    version = timetable.create_version(
        TimetableVersionCreate(
