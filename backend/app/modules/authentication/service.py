...
def create_access_token(user: User) -> str:
    ...
    return jwt.encode(payload, settings.resolved_jwt_secret_key, algorithm=ALGORITHM)


def decode_access_token(token: str) -> dict[str, object]:
    try:
        payload = jwt.decode(token, settings.resolved_jwt_secret_key, algorithms=[ALGORITHM])
...
class AuthenticationService:
    ...
    def refresh(self, refresh_token: str) -> tuple[str, str]:
        ...
        if not stored or stored.revoked_at is not None or expires_at <= now or not stored.user.is_active:
